...
def load_data(data_path, dataset_str, PCA_dim, is_NE=True, n_clusters=20, K=None):
    ...
    print("start Aotoencoder ---------------------------------------")
    # start = time.time()
    # Layers 
    input_dim = features.shape[1] 
    encoding_dim = 1024
    input_layer = Input(shape=(input_dim, ))
    encoder_layer_1 = Dense(4096, activation="tanh")(input_layer)
    encoder_layer_2 = Dense(2048, activation="tanh")(encoder_layer_1)
    encoder_layer_3 = Dense(encoding_dim, activation="tanh")(encoder_layer_2)
    # Model
    encoder = Model(inputs=input_layer, outputs=encoder_layer_3)
    encoded_data = np.array(encoder.predict(features))
    encoded_data = (encoded_data - np.mean(encoded_data)) / (np.std(encoded_data))
    print("End Aotoencoder  ---------------------------------------")
    # print("module time is :", time.time() - start)
    return adj, encoded_data, cells, genes
